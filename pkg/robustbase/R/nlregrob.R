...
## (1) Saving .Random.seed via seed.keep:  Other robustbase functions only
##     do this, when argument 'seed' is specified.
##   MM: I would want
##    a) not to "keep seed at all",  _or_ then at least
##    b) only do what the other robustbase functions do: only "keep"  when
##    extra argument seed is specified.
		     ...
		     optim.control = list(), ...)
{
	...
	tuning.chi.scale <- switch(psi, bisquare = 1.56, lqq = c(0.402, 0.268, 1.5),
				   optimal = 0.405, hampel = c(1.5, 3.5, 8)*0.212)
    ...
    structure(list(call = match.call(), formula = model, nobs = nobs, coefficients = coef,
		     ...
		     tuning.chi = NULL, ...)
{
    ...
    b <- tuning.chi$b
    cc <- tuning.chi$cc
    c <- tuning.chi$c # factor used in objective()
		      ...
		      tolerance = 1e-5, f0 = NULL, ...)
{
    ...
    names(fit) <- rownames(data)
    resid <- y - fit
    quan <- trim( resid/(if ("sigma" %in% pnames) coef["sigma"] else mad(resid)) )$h
