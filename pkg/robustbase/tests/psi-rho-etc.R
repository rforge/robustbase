...
## test derivatives (adapter from lmrob.psifun.R)
...
chkPsiDeriv <- function(m.psi, tol = 1e-4) {
    ...
    c(all.equal(diff(m.psi[,"rho"])/dx, mids(psi), tol=tol[1]), # rho'  == psi
      ...
      all.equal(mids(m.psi[,"wgt'"]), diff(m.psi[,"wgt"])/dx, tol=tol[2])) # wgt' == psi'/x - psi/x^2
}
...
stopifnot(chkPsiDeriv(p.psiFun(x., hampelPsi), tol=c(1e-4, 1e-1)))
