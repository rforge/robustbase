...
\noindent\textbf{Remark: } The factor $\tau^2$ in \eqref{eq:gammatau} was added in
\texttt{robustbase} version \texttt{0.91} to ensure that the equation
reduces to $1$ in the classical case ($\psi(x) = x$).
