...
legend.mod <- list(`SMD.Wtau` = expression(paste('SMD.W',tau)),
                   ...
                   `t3` = expression(t[3]),
                   `t5` = expression(t[5]),
                   `cskt(Inf,2)` = expression(paste('skt(',infinity,',2)')),
                   `cskt(5,2)` = expression('skt(5,2)')
                   )
@ 


\title{Simulations for Wald-type Inference in Robust Regression}
...
test.lm <- drop.levels(subset(test.1, Function == 'lm'), reorder = FALSE)
test.lm$Psi <- NULL
test.lm.2 <- drop.levels(subset(test.lm, Error == 'N(0,1)'), reorder = FALSE)
test.1 <- drop.levels(subset(test.1, Function != 'lm'), reorder = FALSE)
test.2 <- drop.levels(subset(test.1, Error == 'N(0,1)' & Function != 'lm'), 
                      reorder = FALSE)
...
\item \textbf{Scale estimates.} The criteria for scale estimates are all
  ...
  \forall i$.  
\item \textbf{Coefficients.}  The efficiency of estimated regression
...
print(ggplot(drop.levels(subset(test.3, Estimator %in% c("SMD", "MMqE")),
...
print(ggplot(drop.levels(subset(test.1, Estimator %in% c("SMD", "MMqE") & 
                                Psi == 'bisquare'),
                         reorder=FALSE), aes(p/n, q, color = Est.Scale)) +
      stat_summary(aes(x=ratio), fun.y=median, geom='line') + 
      geom_point(aes(shape = factor(n)), alpha = alpha.n) + 
