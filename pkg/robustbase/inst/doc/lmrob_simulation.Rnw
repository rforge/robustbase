...
print(ggplot(test.3, aes(p/n, sdlogsigma.1*sqrt(n), color = Est.Scale)) +
      ...
      ## coord_trans(y = 'sqrt') +
      facet_wrap(~ Psi) +
      ## geom_point(data=test.lm.2, alpha = alpha.n) + 
      stat_summary(data=test.lm.2, aes(x=ratio), fun.y=median, 
...
print(ggplot(test.4, aes(p/n, sdlogsigma.1*sqrt(n),
      ...
      ylab(expression(sd(log(hat(sigma)))*sqrt(n))) +
      facet_wrap(~ Psi) +
      ## geom_point(data=test.lm, alpha = alpha.n) + 
      stat_summary(data=test.lm, aes(x=ratio), fun.y=median, 
                   geom='line', color = 'black') + 
...
print(ggplot(test.2, aes(p/n, f.truncate(emplev_1),
      ...
      ## geom_point(data=test.lm.2, alpha = alpha.n) + 
      ## stat_summary(data=test.lm.2, aes(x=ratio), fun.y=median, geom='line') + 
      g.scale_y_log10_0.05() +
      scale_colour_discrete(name = "Estimator")  +
