...
f.plot.psi <- function(x, psi) {
  ...
  data.frame(x=x, value=.M.psi(x, cc, psi), psi = psi)
}
    ...
    lres <- apply(lresids,3:4,f.aggregate.results <- {
      function(lresid) {
        ...
        if (lcdn[1] != 'estname=lm') {
          ...
          ## A
          lsp2 <- rowSums(.M.psi(lresid,lctrl$tuning.psi,
                                                    lctrl$psi)^2)
          ## B
          lspp <- rowSums(lpp <- .M.psi(lresid,lctrl$tuning.psi,
                                                           lctrl$psi,1))
        ...
        if (grepl('args.method=\\w*(D|T)\\w*\\b', lcdn[1])) { ## SMD or SMDM
          ...
          lwgts <- .M.wgt(lresid,lctrl$tuning.psi, lctrl$psi)
          ...
          lresid <- t(lresid) / lt
          ## A
          lsp2t <- colSums(.M.psi(lresid,lctrl$tuning.psi,
                                                     lctrl$psi)^2)
          ## B
          lsppt <- colSums(.M.psi(lresid,lctrl$tuning.psi,
                                                     lctrl$psi,1))
        } else {
          lsp2t <- lsppt <- NA
        }
...
g <- Vectorize(function(s, theta, mu, ...) {
  lctrl <- lmrob.control(...)
  rho <- function(x)
    .M.chi(x, lctrl$tuning.chi, lctrl$psi, deriv = 0)
