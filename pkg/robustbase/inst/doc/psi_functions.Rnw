...
Unless otherwise stated, the following definitions are given by
\citet{MarRMY06}. 
...
\begin{equation*}
  \rho(x) = \left\{ \begin{array}{cl}
    ...
    \end{array} \right.\;,
\end{equation*}
with derivative $\rho'(x) = 6\psi(x) / k^2$ where,
\begin{equation*}
...
\begin{figure}[h]
  \centering
<<Huber>>=
p.psiFun3(x., huberPsi, ylim=c(-1.3, 6), leg.loc="topright")
@ 
...
\begin{align*}
  \rho(x, k) ={}& 1 - \exp\left(-\frac{1}{2}\left(\frac{x}{k}\right)^2\right) \\
  \psi(x, k) ={}& k^2\rho'(x, k) =
