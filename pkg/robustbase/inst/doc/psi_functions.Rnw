...
\begin{align*}
  \rho_k(x) = {}& \left\{ \begin{array}{ll}
                  x^2 & \mbox{ if } \abs{x} \leq k \\
      2k\abs{x} - k^2 & \mbox{ if } \abs{x} > k
    \end{array} \right. \;,\\
...
\begin{equation*}
  \tilde\rho_k(x) = \left\{ \begin{array}{cl}
    ...
    \end{array} \right.\;,
\end{equation*}
with derivative $\tilde\rho'_k(x) = 6\psi_k(x) / k^2$ where,
\begin{equation*}
...
\begin{align*}
  \tilde\rho_{a, b, r}(x) ={}& \left\{ \begin{array}{ll}
    ...
    \psi_{a, b, r}(x) ={}& \left\{ \begin{array}{ll}
          x & \abs{x} \leq a \\
          a \ \mathrm{sign}(x) & a < \abs{x} \leq b \\
          a \ \mathrm{sign}(x) \frac{r - \abs{x}}{r - b}& b < \abs{x} \leq r \\
          0  & r < \abs{x}
        \end{array} \right.\;,
\end{align*}
...
\begin{equation*}
  \psi_c(x) = \mathrm{sign}(x)\left(-\frac{\varphi'(\abs{x}) + c}
